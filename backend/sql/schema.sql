...
CREATE POLICY "Public can view published tutorials"
  ON tutorials FOR SELECT USING (published = true);
...
AS $$ SELECT role FROM profiles WHERE id = user_uuid; $$;
...
RETURNS BOOLEAN
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'editor')); $$;
...
-- Decrement stock when order moves to processing
